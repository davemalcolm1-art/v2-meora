...
const ScienceCards = () => (
  <section style={{ position: "relative", background: `url(${BG}) center/cover no-repeat`, padding: "120px 0", overflow: "hidden" }}>
    ...
    <div className="sci-wrap" style={{ position: "relative", zIndex: 1, maxWidth: 1200, margin: "0 auto", padding: "0 80px" }}>
      <div style={{ marginBottom: 56 }}>
        <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(255,255,255,0.45)", marginBottom: 20 }}>THE SCIENCE</div>
        <h2 style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, color: "#fff", fontSize: "clamp(36px,4vw,52px)", lineHeight: 1.1, letterSpacing: "-0.02em", margin: 0 }}>
          The science exists.<br />
          <em style={{ fontStyle: "italic", color: "#FF5003" }}>We didn't invent it.</em>
        </h2>
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 16, lineHeight: 1.75, color: "rgba(255,255,255,0.55)", maxWidth: 520, marginTop: 16 }}>
      ...
      <div className="sci-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
        {cards.map((c, i) => (
          <div key={i} className="sci-card" style={{
      ...
      <p style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 400, fontSize: 11, color: "rgba(255,255,255,0.3)", maxWidth: 600, margin: "48px auto 0", lineHeight: 1.6, textAlign: "center" }}>
    ...
    <style>{`
      .sci-card:hover { background: rgba(255,255,255,0.14) !important; transform: translateY(-4px); }
